...
### balance.rs
P5 ✅ 数值调参常量表（第二阶段调参改这一个文件即可）：`INITIAL_EMBER_SHARDS=8`（新玩家初始碎片）、`PORTAL_DELAY_SECS=90.`/`PORTAL_SPAWN`（传送门）、`CREEPER_SPAWN_COUNT=3`/`ZOMBIE_SPAWN_COUNT=5`（怪数）、`SCATTERED_SHARD_COUNT=8`/`SCATTERED_WEAPON_COUNT=2`（散落物资）、`SPAWN_BOUND=16.`/`ENTRY_CLEAR_RADIUS=6.`（刷点范围）、`ZOMBIE_LOOT_SHARD_COUNT=2`（僵尸掉落）、`ZOMBIE_WEAPON_RATIO=(4,3,3)`（剑/矛/空手权重）。合成配方不在此处——`ui::crafting::RECIPES` 已是集中表（8/12/4×shard→sword/spear/tnt）。
...
- **operation 内容随机刷取（P2 ✅）**：`handle_dimension_generation_request` 在 key=OPERATION 生成后调 `spawn_operation_content(commands, &mut rng)`（参数 `ResMut<SystemRng<SmallRng>>`）：`creeper()`×3 + `zombie(ZombieWeapon::roll(rng))`×5 + `item_actor_of(item_stack(key))`×10（8×EMBER_SHARD + 2×SWORD/SPEAR 各半随机），各自 `template_value(Transform::from_translation(random_operation_position(rng)))`。`random_operation_position`：±16 边界（`balance::SPAWN_BOUND`）、y=1、距入口 (0,1,18) ≥6（`balance::ENTRY_CLEAR_RADIUS`）、16 次重试、fallback (0,1,-16)。**P5 起所有数值常量收拢进 `crate::balance`**（见 balance.rs 条目）：怪数/散落数/刷点范围/传送门延迟与位置（`balance::PORTAL_DELAY_SECS`/`PORTAL_SPAWN`）/僵尸掉落与武器权重；dim.rs 仅保留布局常量 `OPERATION_OBSTACLES`。
...
- **玩家与维度子树解耦（P1 ✅）**：玩家 ChildOf `RootNode`（不在维度实体下）；维度生成时若玩家已存在→瞬移（Transform+SpawnPoint+LinearVelocity 清零，背包物品 ChildOf 玩家自然保留），不存在→`player_scene` 新建。**P5 ✅ 初始物资**：新建玩家且目标维度=lobby（即标题屏 Play 的 Init 流程，全 session 唯一一次）→`grant_initial_stash` 预置 `balance::INITIAL_EMBER_SHARDS`×ember_shard 到槽 0（spawn `item_stack`+`StackCount`+ChildOf 玩家，`commands.queue` 闭包里 `world.get_mut::<PlayerInventory>` 写槽）；其他进 lobby 路径（gateway/portal 传送已有玩家）不送。
...
- `Explosion` 事件 + `explode` observer：一次爆炸粒子（bevy_sprinkles）+ 球形范围（radius=power*2）LivingActor 查询，衰减伤害 `7*(x²+x)*power+1`（x=1-距离/半径，圆心最大）+ 方向击退 17*x；**排除爆炸源自身**（`source.direct_entity`——自爆的 creeper 查询时仍有 collider，不排除会写出一条 stale 自伤消息，次帧 `damage` 系统 warn "Could not damage nonexistent living actor"，2026-08-27 修）。`// TODO consider more realistic explosions`（不破坏方块、不连锁）。
...
`damage` 系统：属性减免（DamageTaken.value_for）→ 扣血 → DamageNumber 飘字（头顶 +1.5）→ 击退（Tnua Knockback 动作，可打断当前动作，受 KnockbackTaken 属性）→ HitStun 0.25s → 死亡：**玩家（2026-08-27 死亡屏流程，P3 搜打撤惩罚保留）=清空击退动作；若 LoadingScreen 态（基本不可达：怪随旧维度死光）→回满血+瞬移 SpawnPoint 兜底；否则→插入 `Dead` + `Visibility::Hidden`（尸体免伤/隐藏/脱离怪物感知），非 lobby 维度时清空背包（全槽 despawn），`NextState::set(ActiveOverlay::DeathScreen)`——**不立即重生**，规避延迟伤害刮掉刚回的血（旧流程正是死时立即回满血+warp，加载帧里的迟到伤害把血刮低）；怪物=按 LootTable 在死亡点 +0.5Y 生成 item_actor 后 despawn**（LootTable 每项一个 item_actor，重复条目=多件掉落）。**同帧双重死亡守卫（2026-08-27 修）**：pass 内维护 `dead: Vec<Entity>`，死亡分支执行时 push；后续同目标 `Damage` 命中 `dead`（或实体带 `Dead` 组件）→ 整条跳过。根因：despawn 是延迟命令，第二条 `get_mut` 仍能取到"已死未应用 despawn"的实体 → 死亡分支跑两遍 → 二次 `despawn`/双份掉落/玩家双触发 `Load` 排队 → apply 时 `Entity despawned ... generation+1` panic（详见"bevy 0.19 API 坑"⑨）。注意 `value_for(amount)` 以传入数值为 base、只乘本实体 modifiers（toml 无 damage_taken 键的实体=无 modifier=全额受伤；dummy 靠 `AddMultipliedValue(-1.0)` 归零）。
`Dead` 组件（死亡屏等待标记，2026-08-27）：damage 系统跳过、ai 感知 `Without<Dead>`、尸体 `Visibility::Hidden`。`finalize_respawn`（Update，Dimension 态）：玩家**真正落地 lobby**（overlay ≠ LoadingScreen 且 ActiveDimension == LOBBY）才回满血 + 移除 `Dead`/`HitStun` + `insert(Visibility::Visible)` 恢复可见（**不能 remove Visibility**，见 bevy 0.19 坑"删 Visibility=永久隐藏"）——warp 加载帧里玩家仍是死的，怪打不动。
plugin：`init_resource::<SystemRng<SmallRng>>()` + Update `(damage, creeper::system, zombie::system, finalize_respawn).run_if(in_state(Dimension))` + ai/damage_number 子插件。
...
P2 ✅ 近战怪（新文件）。`Zombie(ZombieState)`（Clone+Component，`#[require(AiTarget, AiPerception, AttackCooldown)]`）；`ZombieState { Idle, Chase, Attack }`（Attack 次帧回 Chase）。`ZombieWeapon { BareHand(default), Sword, Spear }` + `roll(rng: &mut impl RngExt)`（权重取 `balance::ZOMBIE_WEAPON_RATIO`=(4,3,3)：`random_range(0..总和)`，P5 改数据驱动）。打击：剑=`MeleeStrike(6,6,120°,2)`、矛=`MeleeStrike(7,7,30°,2.5)`（均 `LazyLock<MeleeStrike>` 预计算）、空手=距目标 ≤1.5 直接 `Damage` 2-4（`random_range`）+径向击退 5（无扇形）。`zombie(weapon)` 场景：`living_actor(&KEY, false)` + `Collider::cylinder(0.5, 1.7)` + 绿色圆柱网格（srgb(0.15,0.8,0.1)）+ **`template_value(weapon)`**（枚举不能走 bsn patch 语法）+ AiPerception{24,2} + `LootTable({vec![EMBER_SHARD.clone(); balance::ZOMBIE_LOOT_SHARD_COUNT]})`（P5：掉落数走 balance 表）。系统 `zombie::system`（查询排除 Player）：感知→追（speed=movement_speed 属性）→ Chase 且 in_range 且冷却就绪（let-chain）→ 按武器 strike + `cooldown.0.reset()` + 状态 Attack；HitStun 冻结状态推进。⚠️ 无 glTF 模型（绿色圆柱占位）。
...
### ui.rs
**两级状态机**：`GameState { MainMenu(默认), Dimension }` × `ActiveOverlay { Crafting, DeathScreen, GatewayMenu, HeadsUpDisplay, Inventory, LoadingScreen(默认), OptionsAudio, OptionsControls, OptionsLanguage, OptionsMain, OptionsVideo, PauseScreen, TitleScreen }`。
- `process_escaping`（PreUpdate）：Escape 回退栈——HUD→Pause；Crafting/Gateway/Inventory/Pause→HUD；4 个选项子页→OptionsMain；OptionsMain→HUD(维度内)/TitleScreen(菜单)；Loading/Title/DeathScreen 不响应（死亡屏只能按 Respawn 离开）。
...
- `text()`（polygon 字体）；`text_button`（200×20 按钮 + hover/focus/disabled 三态贴图切换）：图像更新是**防御性命令**（`commands.queue(|world| ...)` 内先 `world.get_entity_mut(entity).ok()` 存活检查再 `apply_scene`，查询失败提前返回不用 `.expect`）——因为 Remove observer 会在 DespawnOnExit despawn 时触发（见下方 bevy 0.19 坑 ⑧），原 `commands.entity(...).apply_scene` 会打中已死实体 panic（P4 合成面板实测：3 个禁用按钮 = 3 个 panic）。
...
### ui/death_screen.rs
死亡屏（2026-08-27 新文件，"死亡回城血量不满"修复）。OnEnter(DeathScreen) 在 DimensionViewNode 下生成全屏 flex 居中容器 + 320×140 暗红半透明面板（`DespawnOnExit`）："You died" + Respawn 按钮 + 提示。`respawn_on_click`（按钮上 `NodeInteraction` observer，`Single<(Entity, &SpawnPoint), With<Player>>` + `into_inner()` 解构）：**非 lobby** → `Load::EnterDimension(PortalTravel, LOBBY)`（搜打撤回城）；**lobby 内** → 直接 `insert(Transform=SpawnPoint + LinearVelocity::ZERO)` + 置 HeadsUpDisplay。**此处不复活**——玩家仍带 `Dead`（隐藏/免伤），复活由 living.rs `finalize_respawn` 在真正落地 lobby 后执行（回满血 + 移除 Dead/Visibility/HitStun），warp 加载帧里怪打不到。
...
- **despawn 会触发实体的全部 `On<Remove<T>>` observer**（P4 实测，见 bevy_ecs 测试 `observer_on_remove_during_despawn_spawn_empty`）：`DespawnOnExit`（state 插件，`StateTransition` 阶段——PreUpdate 之后、Update 之前，所以设 `NextState` 当帧 Update 里 `in_state` 已是新状态）递归 despawn 整棵实体树时，每个实体的组件 Remove observer 依次触发（observer 执行时实体组件仍可查询）；observer 里 `commands.entity(...)` 排队的命令在 observer 结束后才 apply——此时实体已释放（generation+1、索引可能已被复用）→ `Entity despawned` 命令错误（默认 handler = panic）。修复模式：`commands.queue(|world: &mut World| { if let Some(mut e) = world.get_entity_mut(entity).ok() { e.apply_scene(...) } })` 防御性 apply + observer 查询 `get().ok()` 提前返回（text_button 已按此修，回归 autotest A/B 验证：原代码 3 panic / 修复后 PASS）。
- **despawn 是延迟命令：同一命令队列里对同一实体的第二个命令必炸**（2026-08-27 双重死亡 panic 实测）：系统 A 排队 `despawn(X)` 后，同帧后续逻辑（甚至**同一系统同一 pass 里的下一条消息**）对 X 再排队任何命令（`despawn`/`ChildOf(X)` 模板/组件插入）→ apply 时第一个 despawn 已 bump generation → 第二个命令报 `Entity despawned: ... generation N+1` panic。且注意：despawn 排队后未 apply 前，`Query::get_mut(X)` **仍能成功**（组件还在）——不能靠"查不到"判断实体已死。修复模式：同一 pass 内记录已处理死亡的实体（`Vec<Entity>`/`HashSet`），后续命中直接 skip（`damage` 系统的 `dead` 守卫即此模式）。
- **`Single` 系统参数：匹配 0 个或 ≥2 个实体时系统被静默跳过（不 panic）**——HUD 等系统依赖此行为在 overlay 切换时安全空转；需要"必须唯一"语义时用 `Option<Single<...>>` 显式判空，或用 `Query::single()`（带 `QuerySingleError`）。
- `Single::into_inner()` 是解构 `&mut` 组件的干净写法（`let (entity, mut inv) = single.into_inner();`）；`let (a, ref mut b) = *single;` 也能过（双重引用 + 传参自动 deref 收窄）。
- `Visibility` 在 0.19 是 **enum**（`Inherited`(默认)/`Hidden`/`Visible`，在 bevy_camera，prelude 有），旧版 struct 的 `visibility.hidden()`/`.is_hidden()` 方法不存在（E0599）。
- **删掉 `Visibility` 组件 = 实体永久隐藏**（2026-08-29 死亡屏 respawn 实测）：`visibility_propagate_system`（PostUpdate）的查询要求实体**带着** `&Visibility` 才会更新其 `InheritedVisibility`；`remove::<Visibility>()` 后传播系统永远不再碰它，`InheritedVisibility` 停在隐藏态 → 再也不渲染。隐藏用 `insert(Visibility::Hidden)`，恢复用 `insert(Visibility::Visible)`（或 `Inherited`），**不要 remove**。验证这类 bug 要查 `InheritedVisibility::get()`（真实渲染态），只查 `Visibility` 组件不够（组件缺失 ≠ Hidden，但效果一样隐身）。
- 查询项里的 `&mut T` 组件做**索引赋值**时绑定必须 `mut`（E0596）：`let (e, mut inv) = query.iter_mut().next().expect(...); inv[0] = Some(x);`（living.rs 死亡分支 `if let Some(mut inventory) = inventory` 同款）。
- `Button` 组件在 **bevy prelude（bevy_picking）**，不存在 `bevy::ui::Button`（E0432）；`Hovered` 在 `bevy::picking::hover::Hovered`（prelude 无）。

## pld/ 数据文件（embers 命名空间）
- `global/actors/embers/`：player.actor.toml（float_height=1, max_health=20, movement_speed=0.5）、creeper.actor.toml（float_height=1, max_health=20, movement_speed=**0.25** 临时降速）、zombie.actor.toml（float_height=1, max_health=30, movement_speed=**0.25** 临时降速）、tnt.actor.toml/item.actor.toml（**空文件**，用默认）。
...
8. ⚠️ Creeper/Zombie 均为圆柱占位模型（Zombie 有 LootTable=2×ember_shard，Creeper 无掉落）；玩家是圆柱占位；死亡屏 UI 极简（2026-08-27 已加，无动画/音效）；地面物品无动画/无消失时限；怪物直线 chase 无寻路（可能卡死在障碍物上）。
...
- P3 死亡惩罚 ✅（用户已实测验收）：operation 死亡=清空背包+回 lobby 惩罚（2026-08-27 起死亡改走死亡屏确认流程，见下方"死亡修复"条；实现见 living.rs `damage` 死亡分支）。
...
- P4 修复：关闭合成面板 panic ✅（2026-08-26）：用户报按 E 开面板、按 Esc 关闭时 panic（`Entity despawned`，generation 不匹配，3 次）。根因 = bevy 0.19 的 despawn 触发全部 Remove observer（见 bevy 0.19 API 坑末条）→ text_button 的 `On<Remove<InteractionDisabled>>` 贴图切换 observer 在面板树被 `DespawnOnExit` 拆除时触发 → observer 排队的 `apply_scene` 命令在实体释放后 apply → panic（禁用按钮各 panic 一次）。修复：text_button 图像更新改防御性命令 + 查询 `get().ok()` 提前返回（ui.rs）。回归 autotest（env 门控，已清除）A/B 验证：原代码精确复现 3 panic，修复后"开面板→验按钮态（9 shards：sword/tnt 可用、spear 禁用）→点合成 sword（9→1）→按钮全禁用→关面板"全程无 panic、exit 0。fmt/clippy/test(34) 通过，已推送（9909622）。
- 双重死亡 panic 修复 ✅（2026-08-27）：用户实测 operation panic（`Entity despawned: 645v1 ... generation 2`，exit 101）。根因 = 同一怪物同帧多条 `Damage`（爆炸批 + 近战；`MeleeStrike` 只排除攻击者，**怪物会互相打到**）→ `damage` 系统里第一条致死伤害排队 despawn 后，第二条 `get_mut` 仍能取到（despawn 未应用）→ 死亡分支跑第二遍 → 二次 `despawn` 排队（用户日志双份掉落=4 条 default-item-stack 警告=2×2）→ apply 时 panic。次要 = creeper 自爆时 `explode` 空间查询含自身（collider 还在）→ stale 自伤消息 → 次帧警告 "Could not damage nonexistent living actor"。修复 = `damage` 加 `dead` 守卫 + `explode` 排除 `direct_entity`（详见 dim.rs / living.rs 条目）。A/B autotest（env 门控临时模块，已清除，main.rs 归零）：同一输入（同帧 2 条致死 Damage 打同一僵尸）无守卫版精确复现 panic（generation 1）+ 掉落翻倍，有守卫版 PASS（despawn 恰好一次、掉落恰好 2、玩家存活、exit 0）。fmt/clippy/test(34) 通过。同日临时平衡调整：creeper/zombie movement_speed 0.5→0.25（怪物移速=玩家一半，便于测试；pld+shp 已同步，shp creeper 镜像原先缺 movement_speed 行已补齐）。⚠️ 提交时 push 因网络连不上 github 失败，两个 commit 暂在本地，网络恢复后 `git push origin main`。
- 死亡修复：死亡屏 + Respawn ✅（2026-08-27 完成，运行时 autotest 验证）：用户报"死亡后返回 lobby 血量没回满"。根因 = 旧流程死亡瞬间回满血+warp，warp 加载帧里的迟到伤害把刚回满的血池刮低（用户建议方案=死亡不立即重生，先出死亡屏，按 Respawn 才重生回满血，结构性绕开延迟伤害）。实现：新 `Dead` 组件（damage 系统免伤、ai 感知 `Without<Dead>`、尸体 `Visibility::Hidden`）；死亡分支不再回血/传送，改插 `Dead` + 置 `ActiveOverlay::DeathScreen`（新 `ui/death_screen.rs`：You died + Respawn 按钮，Esc 不响应）；Respawn → 非 lobby=`Load::EnterDimension(PortalTravel, LOBBY)`，lobby 内=瞬移 SpawnPoint+HUD；新 `finalize_respawn`（living.rs，Update/Dimension）只在**真正落地 lobby**（overlay≠LoadingScreen 且 ActiveDimension==LOBBY）时回满血+移除 Dead/Visibility/HitStun——warp 加载帧里玩家仍是死的，怪打不动。运行时验证（env EMBERS_AUTO_TEST 门控临时代码，验证后已清除，main.rs 归零）：标题→lobby→gateway 进 operation→发 10 shards+血置 1+同帧 3 条 5 伤害（第一条致死，后两条被 dead 守卫/Dead 组件忽略）→死亡屏（Dead 在、Hidden、未回血、背包已清空、恰 1 个按钮）→点 Respawn→回 lobby→Dead 消失、血**恰好**满、背包空，PASS exit 0。踩坑：bevy 0.19 `Visibility` 是 enum（`Visibility::Hidden`，无 `::hidden()`）；查询项 `&mut T` 组件做索引赋值绑定要 `mut`（E0596）；`Button` 组件在 bevy prelude（picking），无 `bevy::ui::Button`（E0432）。fmt/clippy/test(34) 通过，已推送（33e4415）。
  - 死亡修复后续：respawn 后玩家不可见 ✅（2026-08-29，用户实测报 bug）：`finalize_respawn` 用 `remove::<Visibility>()` 恢复可见是错的——bevy 只对**仍带 `Visibility` 组件**的实体传播 `InheritedVisibility`，删组件后玩家 `InheritedVisibility` 永久停在 false。修复 = 改 `insert(Visibility::Visible)`。A/B autotest（加强断言：`Visibility::Visible` + `InheritedVisibility::get()==true` + 满血 + 背包空）：修复前精确复现（`got None`，exit 101），修复后 PASS exit 0；临时代码已清除，main.rs 归零。fmt/clippy/test(34) 通过，已推送（a65ce25）。
- P5 初始物资 + 数值调参 ✅（2026-08-29 完成，运行时 autotest 验证，待用户手动实测）：① 新 `balance.rs` 常量表收拢全部玩法数值（见 balance.rs 条目），dim.rs/zombie.rs 硬编码常量全部改为 `balance::X` 引用（dim.rs 仅留布局常量 `OPERATION_OBSTACLES`）；② 初始物资 `grant_initial_stash`（dim.rs）：仅**新建玩家且目标维度=lobby**（标题屏 Play 的 Init 流程，全 session 唯一一次）预置 `INITIAL_EMBER_SHARDS`=8×ember_shard 到槽 0；其他进 lobby 路径（gateway/portal 传送已有玩家）结构性不送（走的是"玩家已存在→瞬移"分支）。运行时验证（env EMBERS_AUTO_TEST 门控临时代码，已清除，main.rs 归零）：开局槽 0=8 shards→gateway 进 operation 不重送→杀玩家→死亡屏→Respawn 回 lobby 背包空（死亡清包生效、不重送），PASS exit 0。fmt/clippy/test(34)/build 通过，已推送（1638d66）。手动验收清单：新游戏开局 8 碎片→lobby 合成→E gateway 进图→战斗/拾取（僵尸掉 2 碎片、散落 8 碎片+2 武器）→90s 后传送门出现→E 回 lobby→死亡→死亡屏→Respawn→回城后不重发初始物资。
