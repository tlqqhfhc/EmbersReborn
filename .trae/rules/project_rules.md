...
`Dead` 组件（死亡屏等待标记，2026-08-27）：damage 系统跳过、ai 感知 `Without<Dead>`、尸体 `Visibility::Hidden`。`finalize_respawn`（Update，Dimension 态）：玩家**真正落地 lobby**（overlay ≠ LoadingScreen 且 ActiveDimension == LOBBY）才回满血 + 移除 `Dead`/`Visibility`/`HitStun`——warp 加载帧里玩家仍是死的，怪打不动。
...
- `Visibility` 在 0.19 是 **enum**（`Visibility::Visible` / `Visibility::Hidden`），旧版 `visibility.hidden()`/`.is_hidden()` 方法不存在（E0599）。
